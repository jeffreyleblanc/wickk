...
function ifin(b,a){for(i in a){if(b==a[i]){return true}}return false}Array.prototype.remove=function(c){var a,b;if((a=this.indexOf(c))>-1){return([].splice.apply(this,[a,a-a+1].concat(b=[])),b)}};Array.prototype.last=function(){return this[this.length-1]};function makeJO(a){if(a==null){return null}else{if(typeof a=="string"){return $.parseJSON(a)}else{return a}}}function makeWJO(a){if(a==null){return null}else{if(typeof a=="string"){return $.parseJSON(a)}else{return a}}}function sync(c,b){for(var a in c){if(a in b){if($.type(c[a])==="object"){sync(c[a],b[a])}else{c[a]=b[a]}}}}function isEmpty(b){for(var a in b){if(b.hasOwnProperty(a)){return false}return true}}var $cE=null;function C(b){var c=$.type(b);if(ifin(c,["object","array"])){Cappend($.dump(b))}else{Cappend(b)}}function Cappend(a){console.log(a)}var base62alphabet="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";function b62ify(a){a=parseInt(a);var b="";while(a!=0){b=base62alphabet[a%62]+b;a=parseInt(a/62)}return b}function time62(){return b62ify(new Date().getTime())}var global_millis=new Date().getTime();var global_elapsedFrm_millis=0;function update_millis(){var a=new Date().getTime();global_elapsedFrm_millis=a-global_millis;global_millis=a}function millis(){return global_millis}function Emillis(){return global_elapsedFrm_millis}RGX={pattX:/\d+\.\d+/gi,patt29:/[2-9]\.\d+/gi,pattXX:/\d{2}\.\d+/gi,patt01:/[^\d][0-1]\.\d+/gi,decimalize:function(b,a){fpatt=RegExp("\\d+(\\.\\d{1,"+a.toString()+"})?");return b.replace(this.pattX,function(c){return c.match(fpatt)[0]})},decimalize0:function(b,a){floatpatt=RegExp("\\d+(\\.\\d{1,"+a.toString()+"})?");floatpatt0=RegExp("[^\\d]\\d+(\\.0*\\d{1,"+a.toString()+"})?");b=b.replace(this.patt29,function(c){return c.match(floatpatt)[0]});b=b.replace(this.pattXX,function(c){return c.match(floatpatt)[0]});b=b.replace(this.patt01,function(c){return c.match(floatpatt0)[0]});return b},findObj:function(a){patt=/\{[^/{/}]+\}/gi;return a.match(patt)},findPos:function(a){patt=/"pos":\{[^/{/}]+\}/gi;return a.match(patt)},setPos:function(c,a){patt=/"pos":\{[^/{/}]+\}/gi;var b=this;return c.replace(patt,function(d){return b.decimalize(d,a)})},minify:function(d){d=this.setPos(d,1);d=this.decimalize0(d,3);var e=R.genTypes(d);for(var a in e){var c=e[a].toString();fpatt2=RegExp(a,"g");d=d.replace(fpatt2,c)}var b=JSON.stringify(e,null,"");return'{"K":'+b+',"O":'+d+"}"},decompressToObj:function(d){patt=/"K":\{[^/{/}]+\},/gi;var a;var d=d.replace(patt,function(e){a=e.replace('"K":',"").replace("},","}");return""});dict=JSON.parse(a);for(var b in dict){var c=dict[b].toString();fpatt2=RegExp(c,"g");d=d.replace(fpatt2,b)}obj=JSON.parse(d);return obj.O},genTypes:function(f){var e=/"Y":"(\w+?)"/gi;var a=f.match(e);var g={};for(var c=0;c<a.length;c++){g[a[c].replace('"Y":',"").replace(/"/ig,"")]="a"}var d=0;for(var b in g){g[b]=(d+=1)}return g}};(function(a){a.atype=function(b){var c=a.type(b);if(c==="object"){if(a.isFunction(b.ISatype)){return b.type()}}else{if(c==="number"){return(b%1==0)?"int":"float"}}return c};a.ISatype=function(b){var c=a.type(b);if(c==="object"){if(a.isFunction(b.ISatype)){return true}}return false};a.isDerivedFrom=function(c,b){if(!a.ISatype(c)){return false}return c.$derivedFrom(b)};a.isDef=function(b){return !(b===undefined)};a.C=function(b){if(ifin(a.type(b),["object","array"])){console.log(a.dump(b))}else{console.log(b)}}})(jQuery);
